  ...
  watch: {
      checked(val, oldVal) {
          ...
          if (this.checkChangeByUser) {
              ...
              // 解法2：对称差异   两个数组的变动项 => 交集减去并集 => 数组对称差异  操作页面相对好一点
              // const movedKeys = this.sym(val, oldVal);


              // 解法3(推荐):  将 复杂度 n^2 优化为 2n (比如两万条数据，原代码需要比较8亿次的比较，通过优化就只需要8万次比较) 操作页面几乎感受不到卡顿
              // 链接：https://github.com/Little-Gee/blog/issues/15
              const movedKeys = (() => {
                const oldSet = new Set(oldVal);
                const newSet = new Set(val);
                
                return [
                  ...oldVal.filter(v => !newSet.has(v)), // 被移除的
                  ...val.filter(v => !oldSet.has(v))     // 新增的
                ];
              })();
